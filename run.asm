...
;;;;; Basically a simple run and survive game
;;;;; 
;;;;; From the start design in a level layout and then "game engine" that loads those
;;;;; level definitions from memory in a consistent way once the engine is written
;;;;; with the first level, then the extra levels can be made fairly quickly
;;;;;
...
Main_FullReset:
    ; anything that needs to be done on game boot up - like set level to 1
    ld a, 1
    ld (level), a
...
MainGAME_loop:
    ...
    out ($FE), a           ; Any remaining space in the border is "idle" time
...
Alien_1: 
    ...
    ld a, (AlienDirection)
    cp 0
    jp z, selectLastAlien_1

    ld hl, AlienGraphic_8x8_1_PixelMov
    jp doTheChooseAlienShiftSelectLeft

selectLastAlien_1:
    ld hl, AlienGraphic_8x8_1_Last
    jp doTheChooseAlienShiftSelectRight
Alien_2:
    ld a, (AlienDirection)
    cp 0
    jp z, selectLastAlien_2

    ld hl, AlienGraphic_8x8_2_PixelMov
    jp doTheChooseAlienShiftSelectLeft

selectLastAlien_2:
    ld hl, AlienGraphic_8x8_2_Last
    jp doTheChooseAlienShiftSelectRight
...
; then get the initialization code to rotate it using simple srl instrucitons
    defs 96,0   ;7*16 and preset to zero
AlienGraphic_8x8_1_Last   
    defs 16,0   ;7*16 and preset to zero
